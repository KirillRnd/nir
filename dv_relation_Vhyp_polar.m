...
for i=1:40
    ...
    %if J<J_old
    %записываем в файл
    PRevery_Vhyp_best_2(i,:)=pr0; %базис-вектор
    Jevery_Vhyp_best_2(i)=J;     %функционал
    Tevery_Vhyp_best_2(i) = t(end);   %время перелёта
    ANevery_Vhyp_best_2(i) = AN;  %угловая дальность
    OMevery_Vhyp_best_2(i) = 0;  %разность фаз
    Vadd_Vhyp_best_2(i,:) = dV_add;  %добавка скорости 
    %end
...
plot(Tevery_Vhyp_best_2/(2*pi), ANevery_Vhyp_best_2/(2*pi), 'b', 'LineWidth', 1.5);
...
plot(ANevery_Vhyp_best_2/(2*pi), Vadd_Vhyp_best_2(:,1), 'b', 'LineWidth', 1.5, 'DisplayName', '\deltav_x');
%plot(Tevery_Vhyp_best/(2*pi),Vadd_Vhyp_best(:,1), 'DisplayName', 'v_x')
hold on;
Vadd_norm_2 = sqrt(Vadd_Vhyp_best_2(:,1).^2+Vadd_Vhyp_best_2(:,2).^2);
plot(ANevery_Vhyp_best_2/(2*pi), Vadd_Vhyp_best_2(:,2), 'green', 'LineWidth', 1.5, 'DisplayName', '\deltav_y');
plot(ANevery_Vhyp_best_2/(2*pi), Vadd_norm_2, 'k--', 'LineWidth', 1.5, 'DisplayName', '||\deltav|| пролёт');
plot(vhyp_best_minimum_line(2,1:105)/(2*pi), vhyp_best_minimum_line(6,1:105)/V_unit, 'r', 'LineWidth', 1.5, 'DisplayName', '||\deltav|| опт. гипызбыток')
hold off;
grid;
...
%%
figure(4)
Vadd_angle_2 = atan2(Vadd_Vhyp_best_2(:,2),Vadd_Vhyp_best_2(:,1));
plot(Tevery_Vhyp_best_2/(2*pi),Vadd_angle_2)
grid;
xlabel("Время перелёта, годы")
...
hold on;
plot(Tevery_Vhyp_best_2/(2*pi),Jevery_Vhyp_best_2,'k--', 'LineWidth', 1.5, 'DisplayName', 'J задача 3')
hold off;
xlim([1,8])
legend('Location','best');
grid;
...
plot(ANevery_Vhyp_best_2/(2*pi),PRevery_Vhyp_best_2(:,1),'LineWidth', 1.5,  'DisplayName', 'p_r_x')
hold on;
plot(ANevery_Vhyp_best_2/(2*pi),PRevery_Vhyp_best_2(:,2),'cyan', 'LineWidth', 1.5, 'DisplayName', 'p_r_y')
hold off;
legend('Location','best');
grid;
