...
A0=2*h0*r0-(r0'*V0)*V0+mug*r0/norm(r0);
dtds=norm(r0)/sqrt(-2*h0);
Vs0 = V0*dtds;
...
%%
px=prpV2pXpVXShperlingBurde(r0,Vs0,pr0,pv0)/10;
%px=[0, 0, 0, 0, 0, 0];
pX0=px(1:3)';
pVX0=px(4:6)';
y0 = [r0; Vs0; pX0; pVX0;tau0;A0;h0];

a_0=(pVX0*(r0'*r0) + (Vs0*Vs0')*pVX0)/dtds/(-2*h0);
...
elseif terminal_state == 't'
    options = odeset(options, 'Events',@(s, y) eventIntegrationTrajStopTimeShperlingBurde(s, y, time0,maxtime, t_end_0));
end


%максимальное время интегрирования
s_span=linspace(0,(n+angle)*20,10000);
[s,y] = ode113(@(s,y) integrateTraectoryShperlingBurde(s,y),s_span,y0, options);

XX = y(:, 1:3);
PVX = y(:, 10:12);
rr=zeros(length(XX),3);
t = zeros(length(XX),1);
DTDS = zeros(length(XX),1);
...
for i = 1:length(XX)
    ...
    tau=y(i, 13);
    t_tmp=T_unit*(tau-(r'*Vs)/norm(r)/sqrt(-2*h))/(24*60*60);
    t(i)=t_tmp;
    DTDS(i)=dtds;
