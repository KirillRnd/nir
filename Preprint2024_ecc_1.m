...
%%
savefilename = 'mat-files/Preprint2024_ecc_1.mat';
k_coef_range=0.4:0.1:2.5;
a_coef_range=0.9:0.01:1.1;
ex_coef_range=-0.15:0.01:0.15;
N_count_1 = length(k_coef_range);
N_count_2 = length(a_coef_range);
N_count_3 = length(ex_coef_range);
...
for j1 = 21:N_count_1
    for j2 = 1:N_count_2
        ...
        a_coef = a_coef_range(j2);
        ex_coef = ex_coef_range(j3);
        ...
        ey = 0;
        ex = ex_coef;
        p = a_coef*(1-ex^2-ey^2);
        %p=1;
        X_2 = equitoctial2decart([p;ex;ey;ix;iy;L], mug);
        start_pos_3 = X_2(1:3)';
        start_vel_3 = X_2(4:6)';
        
        R = calculateRotMatrix(start_pos_3,start_vel_3);
        t_end = get_t_end(a_input);
        [pr_0,pv_0] = get_adjoint(a_input);
        z = [pr_0;pv_0];
        [ex,ey,p,a] = integrate_iO_equinoctial(R, start_pos,start_vel,z,t0,t_end);
...
function [ex,ey,p,a] = integrate_iO_equinoctial(R_iO, start_pos,start_vel,zf,t0,t_end)
pr = R_iO^(-1)*zf(1:3);
pv = R_iO^(-1)*zf(4:6);
