...
%%
savefilename = 'mat-files/Preprint2024_8_2.mat';
...
O_massive_error2(O_massive_error2>pi)=-2*pi+O_massive_error2(O_massive_error2>pi);
O_massive_error2(7,:,:) = 0;

i_std = sqrt(sum(i_massive_error.^2, 'all')/(numel(i_massive_error)-1));
a_std = sqrt(sum(a_massive_error.^2, 'all')/(numel(a_massive_error)-1));
O_std = sqrt(sum(O_massive_error2.^2, 'all')/(numel(O_massive_error2)-1-numel(O_massive_error2(7,:,:))));
disp(['i_std=', num2str(i_std)]);
disp(['a_std=', num2str(a_std)]);
disp(['O_std=', num2str(O_std)]);

i_std_alongO = zeros(N_count_3, 1);
for j3 = 1:N_count_3
    i_std = sqrt(sum(i_massive_error(:,:,j3).^2, 'all')/(numel(i_massive_error(:,:,j3))-1));
    i_std_alongO(j3) = i_std;
end
plot(O_coef_range, 180*i_std_alongO/pi)
xticks([O_coef_range(1) (O_coef_range(1)+O_coef_range(end))/2 O_coef_range(end)])
xticklabels({'0','\pi','2\pi'})
grid;
xlabel('ДВУ, радианы')
ylabel('Ошибка наклонения, градусы')
