...
%pr2_0 = 100*pr_0;
%pv2_0 = 100*pv_0;
pr2_0 = [-2.7783,-3.7971,0.0000];
pv2_0 = [-4.8130, -1.2633,0.0000];

y0_double = cat(2,start_pos_2,start_vel_2,pr_0,pv_0,start_pos_2,start_vel_2,pr2_0,pv2_0,0,0,0)';
...
fmincon_traj_fun=@(z)fmincon_traj(z,y0_double,yf,t0,T_i, a_max);
...
[t,y] = ode15s(@(t,y) DoubleIntegration3D_withJ_TH(t,y,a_max), tspan,y0_double,options);

J = y(end,25);       %функционал
AN_final = y(end,26);%угловая дальность
J_linear = y(:,27);       %функционал линейный
...
plot3(y(:, 13), y(:, 14), y(:, 15), 'magenta', 'LineWidth', 1);
...
plot3(mars_r_f_target(1), mars_r_f_target(2), mars_r_f_target(3),'rO')
...
for i = 1:length(break_points) + 1
    ...
    if length(break_points) == 0
        plot3(y(1:end,13), y(1:end,14), y(1:end,15), color, 'LineWidth', 2);
    else
        if i == 1
            % Первая часть до первого разрыва
            plot3(y(1:break_points(i),13), y(1:break_points(i),14), y(1:break_points(i),15), color, 'LineWidth', 2, 'DisplayName',disp_name);
        elseif i == length(break_points) + 1
            % Последняя часть после последнего разрыва
            plot3(y(break_points(i-1)+1:end, 13), y(break_points(i-1)+1:end, 14), y(break_points(i-1)+1:end, 15), color, 'LineWidth', 2, 'DisplayName',disp_name);
        else
            % Промежуточные части
            plot3(y(break_points(i-1)+1:break_points(i), 13), y(break_points(i-1)+1:break_points(i), 14),y(break_points(i-1)+1:break_points(i), 15), color, 'LineWidth', 2, 'DisplayName',disp_name);
        end
    end
...
box off;
%%
function [c, ceq] = fmincon_traj(z,y0,yf,t0,t_end, a_max)
...
[~,y] = ode15s(@(t,y) DoubleIntegration3D_withJ_TH(t,y, a_max), tspan,y0,options);
c =[];
ceq = (y(end,13:18)-yf(1:6));
end
function dydt = DoubleIntegration3D_withJ_TH(~,y, a_max)
...
dydt(13:15)=v_2;
dydt(16:18)=-mug*r_2/nr_2^3+a_star*e_star+0.5*pv;
dydt(19:21)=mug*pv_2/nr_2^3 - mug*(3*(r_2*r_2')/nr_2^(5))*pv_2;
dydt(22:24)=-pr_2;

dydt(25)=(norm(pv)^2)/2; %dJdt
dydt(26)=(r(1)*v(2) - r(2) * v(1)) / (norm(r)^2); %dTHdt
dydt(27)=a_star; %dJdt
end