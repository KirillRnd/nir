...
%%
savefilename = 'mat-files/Preprint2024_6.mat';
load(savefilename)
for j2 = 1:N_count_2
    for j1 = 1:N_count_1
        for j3 = 1:N_count_3
        ...
        delta_i_0 = abs(i_coef/delta_i_coef);
        if abs(a_massive_error(j1,j2-1,j3)) < 1e-4
            a_input_0 = a_massive(j1,j2-1,j3);
        else
            a_input_0 = a_rel;
        end
            
        x0 = [delta_i_0,a_input_0];
        try
            fmincon_fun=@(x)fmincon_find_a_i(x, i_coef,k_coef, O_coef, start_pos,start_vel,t0);
            ...
            i_massive_error(j1,j2,j3) = 1000;
            a_massive_error(j1,j2,j3) = 1000;
        end
        save(savefilename)
        end
    end
...
grid on;
box off;
function [c, ceq] = fmincon_find_a_i(x, i_target,a_target, O_target, start_pos,start_vel,t0)
c =[];
i_input = x(1);
a_input = x(2);
%наклонение
