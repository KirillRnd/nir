...
figure(24);
Jevery_a_fix = Jevery_a;
Jevery_a_fix(Jevery_a_fix==0)=nan;

s = surf(da, ANevery_a/(2*pi), Jevery_a_fix, 'FaceAlpha',FaceAlpha);
s.EdgeColor = 'none';
hold on;
contour3(repmat(da, L2,1), ANevery_a/(2*pi), Jevery_a_fix,'ShowText','on', 'HandleVisibility','off');
hold off;
...
s = surf(repmat(da, L2,1), ANevery_a/(2*pi), Tevery_a./T_planets, 'FaceAlpha',FaceAlpha);
s.EdgeColor = 'none';
hold on;
contour3(repmat(da, L2,1), ANevery_a/(2*pi), Tevery_a./T_planets,'ShowText','on', 'HandleVisibility','off');
hold off;
...
s = surf(PVevery_a(:,:,1), PVevery_a(:,:,2), ANevery_a/(2*pi), 'FaceAlpha',FaceAlpha);
s.EdgeColor = 'none';
hold on;
for j = 1:20:L3
     pos_i = 5;
     ...
     h = text(PVevery_a(pos_i,j,1), PVevery_a(pos_i,j,2), ANevery_a(pos_i,j)/(2*pi), text_label,'HorizontalAlignment','center',...
...
s = surf(PRevery_a(:,:,1), PRevery_a(:,:,2), ANevery_a/(2*pi), 'FaceAlpha',FaceAlpha);
s.EdgeColor = 'none';
hold on;
for j = 1:20:L3
     pos_i = 5;
     ...
     text_label = num2str(da(j));
     h = text(PRevery_a(pos_i,j,1), PRevery_a(pos_i,j,2), ANevery_a(pos_i,j)/(2*pi), text_label,'HorizontalAlignment','center',...
    'VerticalAlignment','Bottom','FontName','consolas','FontSize',9); 
end
hold off;
zlabel('Угловая дальность, безразм.')
