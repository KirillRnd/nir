...
%%
savefilename = 'mat-files/Preprint2024_4.mat';
load(savefilename)
for j1 = 17:N_count_1
    for j2 = 1:N_count_2
        for j3 = 1:N_count_3
        ...
        try
            options_fsolve = optimoptions('fsolve','Display','off');
            fsolve_fun=@(k)fsolve_find_a_fix(k, a_target,R_iO, start_pos,start_vel,t0,t_end);
            [a_fix, fval] = fsolve(fsolve_fun, a_target, options_fsolve);
            a_massive(j1,j2,j3) = a_fix;
            a_massive_error(j1,j2,j3) = fval;
        catch
            continue
        end
        save(savefilename)
...
%%
i_target_massive = zeros(N_count_1,N_count_2,N_count_3);
i_input_massive = zeros(N_count_1,N_count_2,N_count_3);
for j1 = 1:N_count_1
    for j2 = 1:N_count_2
        for j3 = 1:N_count_3
        k_coef = k_coef_range(j1);
        ...
        i_input_massive(j1,j2,j3) = i_coef/delta_i_coef;
        delta_i = atan(i_coef/delta_i_coef);
        n_rot_i = [1; 0; 0];
        q_rot_i = [cos(delta_i/2); n_rot_i*sin(delta_i/2)];
        R_i0 = quat2rotm(q_rot_i');
        ...
        t_end = T_right(j1);
        a_target = k_coef;
        a_fix = a_massive(j1,j2,j3);
        try
            [pr_0,pv_0] = get_adjoint(a_fix);
            z = [pr_0;pv_0];
            [i,O,p,a,J,AN] = integrate_iO(R_iO, start_pos,start_vel,z,t0,t_end);
            i_target_massive(j1,j2,j3) = i;
        catch
            continue
        end
        save(savefilename)
...
%%
options_fsolve = optimoptions('fsolve','Display','off');
fsolve_fun=@(k)fsolve_find_a_fix(k, a_rel,R_iO, start_pos,start_vel,t0,t_end);
[a_fix, fval] = fsolve(fsolve_fun, 1.4289, options_fsolve);
%%
%k = 2.5;
k = k_coef_range(22);
j2 = 1;
